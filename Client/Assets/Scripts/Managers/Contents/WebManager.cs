...
public class WebManager
{
	...
	public string chattingBaseUrl { get; set; } = "https://localhost:5000/api";
	...
	public void SendChattingPostRequest<T>(string url, object obj, Action<T> res)
	{
		Managers.Instance.StartCoroutine(CoSendChattingWebRequest(url, UnityWebRequest.kHttpVerbPOST, obj, res));
	}
	IEnumerator CoSendChattingWebRequest<T>(string url, string method, object obj, Action<T> res)
	{
		// obj : 해당하는 json오브젝트를 넣어주는 곳

		string sendUrl = $"{chattingBaseUrl}/{url}";
		byte[] jsonBytes = null;
		if (obj != null)
		{
			string jsonStr = Newtonsoft.Json.JsonConvert.SerializeObject(obj);
